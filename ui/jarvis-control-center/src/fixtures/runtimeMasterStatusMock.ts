export const runtimeMasterStatusMock = {
  report_version: 'master_status_v1',
  updated_at_utc: '2026-06-01T18:57:58.1083112+00:00',
  data_root: '/mnt/d/HermesData',
  overall_status: 'warning',
  current_focus: 'consolidation: balanced, trading, documentation, process, software',
  active_domains: ['documentation', 'process', 'research', 'software', 'trading'],
  queued_tasks: 4,
  last_nightly_run: '2026-05-31T21:00:02.0816608+00:00',
  last_autonomous_loop: '2026-06-01T15:18:36.3732613+00:00',
  last_meta_review: '2026-06-01T15:18:35.7665691+00:00',
  learning_strategy: 'consolidation',
  trusted_knowledge: 3,
  weak_knowledge: 18,
  deprecated_knowledge: 2,
  average_quality_score: 0.57,
  average_trust_score: 0.66,
  knowledge_health: 'needs_consolidation',
  knowledge_trend: 'baseline',
  evidence_coverage: 1,
  validation_coverage: 1,
  contradiction_count: 0,
  human_reviewed_items: 0,
  trust_distribution: {
    weak: 100,
    promising: 33,
    validated: 0,
    trusted: 0,
  },
  pending_reviews: 3,
  approved_reviews: 0,
  rejected_reviews: 0,
  needs_more_evidence_reviews: 0,
  deferred_reviews: 0,
  review_coverage: 0,
  top_review_priorities: [
    'high:trading:trading:breakout_continuation:trust=0.6317:human_review_can_unlock_validated_trust',
    'high:trading:trading:ema_pullback:trust=0.6317:human_review_can_unlock_validated_trust',
    'high:documentation:documentation:doc_docs_architecture_hermes_signal_outcome_tracking_v1_md:trust=0.4971:review_for_quality_gate',
  ],
  validation_plans_open: 50,
  validation_tasks_pending: 20,
  trusted_candidate_count: 0,
  knowledge_items_needing_oos: 24,
  knowledge_items_needing_source_check: 41,
  top_goal: 'improve_trading_robustness',
  active_goals: [
    'improve_trading_robustness',
    'reduce_overfit_risk',
    'expand_knowledge_sources',
    'improve_cognitive_memory_quality',
    'maintain_storage_health',
    'prepare_multi_domain_learning',
    'improve_autonomous_planning_quality',
    'improve_research_efficiency',
    'improve_knowledge_quality',
    'reduce_low_confidence_knowledge',
  ],
  blocked_goals: [
    'improve_trading_robustness',
    'reduce_overfit_risk',
    'improve_cognitive_memory_quality',
    'improve_autonomous_planning_quality',
  ],
  goal_progress_summary: {
    maintain_storage_health: 0.5866,
    prepare_multi_domain_learning: 0.54,
    expand_knowledge_sources: 0.5384,
    improve_cognitive_memory_quality: 0.22,
    reduce_overfit_risk: 0.14,
    improve_research_efficiency: 0.14,
    improve_autonomous_planning_quality: 0.06,
    improve_trading_robustness: 0.05,
  },
  goal_warnings: [
    'improve_trading_robustness:blocked:no_robust_strategies',
    'reduce_overfit_risk:blocked:hypotheses_without_validation_queue',
    'improve_autonomous_planning_quality:blocked:research_queue_empty',
  ],
  top_blockers: [
    'high:quality_risk:no_robust_strategies',
    'high:data_gap:oos_data_missing',
    'high:maintenance:storage_cleanup_candidates',
    'medium:knowledge_gap:documentation_gaps',
  ],
  next_recommended_actions: [
    'run_overfit_report:0.7762:planned_run_overfit_report_c08cf73f9dc7',
    'run_strategy_research:0.6722:planned_run_strategy_research_04e1b9ac087d',
    'run_storage_hygiene:0.6098:planned_run_storage_hygiene_ad2f1cea368c',
    'scan_documentation_domain:0.5648:planned_scan_documentation_domain_6fc492c10390',
  ],
  supervisor: {
    running: false,
    status: 'stopped_by_stop_request',
    next_action: 'safe_stop_requested',
  },
  scheduler: {
    enabled_jobs: 18,
    active_jobs: 0,
    failed_jobs: 0,
    next_job: 'health_snapshot',
  },
  resource_status: {
    action: 'continue',
  },
  storage_status: {
    cleanup_candidates: 77202,
    storage_root: '/mnt/d/HermesData',
  },
  trading_domain: {
    robust_strategies: 0,
    demo_bot_candidates: 0,
    rejected_candidates: 14720,
    high_risk_strategies: 14848,
    scalping_asset: 'XAUUSD',
    scalping_candidates_total: 20,
    scalping_robust_candidates: 4,
    scalping_final_candidates: 1,
    best_scalping_candidate: 'scalp_xauusd_1070720f16',
    scalping_monte_carlo_health: 'ok',
    scalping_parameter_sensitivity_health: 'ok',
    scalping_regime_validation_health: 'ok',
    ctrader_bot_specs_ready: 0,
    signal_agent_specs_ready: 0,
  },
  scalping_asset: 'XAUUSD',
  scalping_candidates_total: 20,
  scalping_robust_candidates: 4,
  scalping_final_candidates: 1,
  best_scalping_candidate: 'scalp_xauusd_1070720f16',
  scalping_monte_carlo_health: 'ok',
  scalping_parameter_sensitivity_health: 'ok',
  scalping_regime_validation_health: 'ok',
  ctrader_bot_specs_ready: 0,
  signal_agent_specs_ready: 0,
  demo_signal_feed_status: 'demo_signals_ready',
  demo_signals_available: true,
  latest_demo_signal_count: 2,
  demo_signal_feed_health: 'ok',
  demo_signal_feed_mode: 'simulated_demo_signal',
  forward_test_status: 'observation_ready',
  forward_test_mode: 'demo_signal_observation',
  forward_test_assets: ['EURUSD', 'XAUUSD'],
  forward_test_signals_observed: 0,
  forward_test_health: 'ok',
  forward_test_requires_human_review: true,
  no_auto_trading: true,
  human_review_required: true,
  broker_orders_enabled: false,
  live_trading_enabled: false,
};
