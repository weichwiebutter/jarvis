...
""";
    ...
    private void WriteGeneratedSource(string packageJson, string botReleaseId, string botVersion, string strategyPackageVersion, string embeddedChecksum)
    {
        var generatedPath = OutputSourcePath;
        Directory.CreateDirectory(Path.GetDirectoryName(generatedPath) ?? _runtimeRoot);
        var escapedJson = EscapeForCSharpString(packageJson);
