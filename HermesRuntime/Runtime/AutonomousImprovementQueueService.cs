...
public sealed class AutonomousImprovementQueueService
{
    ...
    private static IReadOnlyList<AutonomousImprovementTask> BuildTasks(IReadOnlyList<string> warnings, KnowledgeValidationAuditReport audit)
    {
        ...
        foreach (var warning in warnings.Distinct(StringComparer.OrdinalIgnoreCase))
        {
            ...
            {
                "oos_data_missing" => NewTask(warning, "OOS-/Walk-Forward-Validierung planen", "trading", "high", "Die Audit-Kette meldet fehlende OOS-Absicherung.", "Weitere Walk-Forward-/OOS-Läufe planen.", false, true, "Innerhalb des nächsten Validierungszyklus"),
                "knowledge_validation_queue_missing" => NewTask(warning, "Validation Queue prüfen/reparieren", "research", "high", "Offene Validation Plans werden nicht in Queue-Arbeit überführt.", "Validation Queue befüllen oder Routing reparieren.", false, true, "Sofort"),
                "hypotheses_without_validation_queue" => NewTask(warning, "Hypothesen in Queue überführen", "research", "high", "Hypothesen hängen ohne Queue-Arbeit.", "Offene Hypothesen in die Validierungswarteschlange überführen.", false, true, "Sofort"),
                "no_robust_strategies" => NewTask(warning, "Research-/Robustness-Lauf planen", "trading", "high", "Es gibt noch keine robuste Strategie.", "Nächsten Robustness-/Research-Lauf planen.", false, true, "Beim nächsten Research-Fenster"),
                "storage_cleanup_candidates" => NewTask(warning, "Cleanup-Plan aktualisieren", "process", "low", "Speicherbereinigung wäre sinnvoll.", "Cleanup-Plan bei Bedarf aktualisieren.", false, true, "Bei Speicherbedarf"),
                _ => null
            };
