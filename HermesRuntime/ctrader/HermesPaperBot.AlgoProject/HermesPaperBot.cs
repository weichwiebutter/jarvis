...
public sealed class HermesPaperBot
{
        ...
        string[] signalWarnings)
    {
        ...
        return new RuntimeStepResult
        {
            ...
            MarketContext = context,
            MarketContextSeen = true,
            RestoreState = GetRestoreDiagnostics().State,
            RestoreReason = GetRestoreDiagnostics().Reason,
            RestoreSnapshotValid = GetRestoreDiagnostics().SnapshotValid,
            RestoreFreshStateUsed = GetRestoreDiagnostics().FreshStateUsed,
            RestoreActiveTradeCount = GetRestoreDiagnostics().ActiveTradeCount,
            RestoreFirstActiveSignalId = GetRestoreDiagnostics().FirstActiveSignalId,
            RestoreFirstActiveEntry = GetRestoreDiagnostics().FirstActiveEntry,
            RestoreFirstActiveSl = GetRestoreDiagnostics().FirstActiveSl,
            RestoreFirstActiveTp = GetRestoreDiagnostics().FirstActiveTp,
            PackageLoaded = _lastConfiguration?.CloudEmbeddedReleasePackage is not null,
            SignalPackageLoaded = HasEmbeddedSignalPackage(),
            SignalCount = GetEmbeddedSignalCount(),
            SignalPackageJsonLength = GetEmbeddedSignalPackageJsonLength(),
    ...
    private RuntimeStepResult PersistRuntimeResult(RuntimeStepResult runtimeResult)
    {
        ...
        if (!loggingOk || !timerLoggingOk || !summaryOk)
        {
            ...
            return new RuntimeStepResult
            {
                ...
                TimerTickCount = timerLogging.TickCount,
                SessionStartedAt = timerLogging.SessionStartedAtUtc,
                LastTimerAt = timerLogging.LastTimerAtUtc,
    ...
    private (string State, string Reason, bool SnapshotValid, bool FreshStateUsed, int ActiveTradeCount, string FirstActiveSignalId, decimal? FirstActiveEntry, decimal? FirstActiveSl, decimal? FirstActiveTp) GetRestoreDiagnostics()
    {
        var restore = _lastStateRestoreResult;
        return (
            restore?.RestoreState ?? "unknown",
            restore?.RestoreReason ?? string.Empty,
            restore?.RestoreSnapshotValid ?? false,
            restore?.RestoreFreshStateUsed ?? false,
            restore?.RestoreActiveTradeCount ?? 0,
            restore?.RestoreFirstActiveSignalId ?? string.Empty,
            restore?.RestoreFirstActiveEntry,
            restore?.RestoreFirstActiveSl,
            restore?.RestoreFirstActiveTp);
    }
}
